...
class CompletionsBaseAgent(BaseAgent):

    def __init__(self, **kwargs: Any):
        if "llm_engine" not in kwargs:
            kwargs["llm_engine"] = OpenAICompletionsEngine(OpenAIModelConfig(
                model_name="gpt-5-mini",
                provider="completions_api",
                endpoint="https://api.openai.com/v1",
                api_key=os.getenv("OPENAI_API_KEY"),
            ))
        assert isinstance(kwargs["llm_engine"], OpenAICompletionsEngine), "llm_engine must be an instance of OpenAICompletionsEngine"
        if "action_executor" not in kwargs:
            kwargs["action_executor"] = CompletionsActionExecutor()
        assert isinstance(kwargs["action_executor"], CompletionsActionExecutor), "action_executor must be an instance of CompletionsActionExecutor"
    ...
    def init_tools(self):
        ...
        self.oai_tools = [t.app_tool.to_open_ai() for t in tool_values]  # pyright: ignore[reportAttributeAccessIssue]
        self.action_executor.update_tools(self.tools)
        return tool_values

    def build_history_from_logs(self, exclude_log_types: list[str] = []) -> list[dict[str, Any]]:
        """
        ...
        for i, log in enumerate(self.logs):
            ...
            if (
                ...
                history.append({"role": "system", "content": log.content})
                continue
            else:
                content_for_llm = log.get_content_for_llm()
                attachments_for_llm = None
    ...
    def step(self):
        ...
        agent_memory = self.build_history_from_logs(
            exclude_log_types=["thought", "action", "rationale"]
        )
        ...
        if "output_tokens_details" in metadata:
